...
class IrActionsReport(models.Model):
    ...
    def _render_qweb_html(self, report_ref, res_ids=None, data=None):
        ...
        return super(IrActionsReport, self)._render_qweb_html(report_ref, res_ids, data=data)

    def _render_qweb_pdf(self, report_name, res_ids=None, data=None):
        if report_name != 'l10n_br_eletronic_document.main_template_br_nfe_danfe':
            return super(IrActionsReport, self)._render_qweb_pdf(report_name, res_ids, data=data)

        nfe = self.env['eletronic.document'].search([('id', 'in', res_ids)])

        nfe_xml = base64.b64decode(nfe.nfe_processada or nfe.xml_to_send)
        ...
        if cce_list:
            for cce in cce_list:
                cce_xml = base64.b64decode(cce.datas)
