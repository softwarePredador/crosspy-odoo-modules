...
class ExportNfe(models.TransientModel):
    ...
    def _save_zip(self, xmls, pdfs):
        ...
        for xml in xmls:
            filename = os.path.join(tmp, xml['name'].replace('/', ''))
            with open(filename, 'w') as xml_file:
                xml_file.write(xml['content'])
            zip_file.write(filename, xml['name'].replace('/', ''))
        for pdf in pdfs:
            filename = os.path.join(tmp, pdf['name'].replace('/', ''))
            with open(filename, 'wb') as pdf_file:
                pdf_file.write(pdf['content'])
            zip_file.write(filename, pdf['name'].replace('/', ''))
        zip_file.close()
        zip_base64.seek(0)
    ...
    def nfse_export(self):
        ...
        for invoice in invoice_ids:
            if invoice.nfe_processada:
                xmls.append({
                    'content': base64.b64decode(invoice.nfe_processada).decode(),
                    'name': invoice.nfe_processada_name
                })
            if invoice.nfse_pdf:
                pdfs.append({
                    'content': base64.b64decode(invoice.nfse_pdf),
            ...
            if invoice.model == 'nfe':
                ...
                danfe, dummy = self.env.ref(report_service)._render_qweb_pdf(danfe_report.report_name, [invoice.id])
