...
class AccountMove(models.Model):
    ...
    edoc_ids = fields.One2many('eletronic.document', 'move_id', string='Edis')
    ...
    def _compute_nfe_number(self):
        ...
        for item in self:
            ...
            #     [('move_id', '=', item.id)])
            if item.edoc_ids:
                item.nfe_number = item.edoc_ids[0].numero
            else:
                item.nfe_number = 0
    ...
    def _validate_for_eletronic_document(self):
        ...
        for move in self:
            ...
            if not move.company_id.partner_id.legal_name:
            ...
            if not move.company_id.partner_id.state_id:
                ...
                if not move.company_id.partner_id.state_id.ibge_code:
            ...
            if not move.company_id.partner_id.city_id:
                ...
                if not move.company_id.partner_id.city_id.ibge_code:
            ...
            if not move.company_id.partner_id.country_id:
                ...
                if not move.company_id.partner_id.country_id.ibge_code:
            ...
            partner = (move.partner_id and move.partner_id.commercial_partner_id) or move.partner_id
            for eletr in doc_lines:
                ...
                if move.company_id.l10n_br_accountant_id and not \
                        move.company_id.l10n_br_accountant_id.vat:
            ...
            if has_services:
                cod_municipio = '%s%s' % (
                    move.company_id.state_id.ibge_code,
                    move.company_id.city_id.ibge_code,
                )
            ...
            if self.fiscal_position_id.nfse_consumidor == False:
                ...
                if partner.vat != '000.000.000-00':
                    errors.append('CNPJ/CPF Consumidor')

            if partner.country_id == self.env.ref('base.br') and len(errors) > 0:
                msg = "\n".join(
    ...
    def _prepare_eletronic_doc_vals(self, invoice_lines, numero_nfe):
        ...
        if invoice.commercial_partner_id.is_company:
            if invoice.commercial_partner_id.inscr_est:
                ind_ie_dest = '1'
    ...
    def action_create_eletronic_document(self):
        for move in self:
            invoice_lines = move.invoice_line_ids.filtered(
                lambda x: x.display_type == 'product' and x.move_id.is_invoice(True) and not x.is_delivery_expense_or_insurance()
            )
    ...
    def action_post(self):
        ...
        if not self.env.user.has_group('l10n_br_eletronic_document.group_auto_edoc_confirm'):
            return super().action_post()
        moves._validate_for_eletronic_document()
        super(AccountMove, self).action_post()
        moves.action_create_eletronic_document()
        self.env.cr.commit()
        if self.env.user.has_group('l10n_br_eletronic_document.group_auto_edoc_confirm'):
            edocs = self.env['eletronic.document'].search(
                [('move_id', 'in', moves.ids), ('state', '=', 'draft')])
            if edocs:
                edocs.action_send_eletronic_invoice()
        return True
...
class AccountMoveLine(models.Model):
    ...
    def get_eletronic_line_vals(self):
        ...
        cfop = fiscal_pos.l10n_br_cfop_id.code or '5102'
        ...
        if self.move_id.move_type in ['in_invoice', 'out_refund']:
            ...
            if self.move_id.company_id.state_id == self.move_id.commercial_partner_id.state_id:
                cfop = '5' + cfop[1:]
            elif self.move_id.commercial_partner_id.country_id == self.env.ref('base.br'):
                cfop = '6' + cfop[1:]
            else:
                cfop = '7' + cfop[1:]
