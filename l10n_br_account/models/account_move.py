...
class AccountMove(models.Model):
    ...
    def _compute_l10n_br_delivery_amount(self):
        move_with_delivery = self.line_ids.filtered(lambda x: x.l10n_br_is_delivery).mapped('move_id')
        (self - move_with_delivery).l10n_br_delivery_amount = 0
        for item in move_with_delivery:
    ...
    def _compute_l10n_br_expense_amount(self):
        move_with_expense = self.line_ids.filtered(lambda x: x.l10n_br_is_expense).mapped('move_id')
        (self - move_with_expense).l10n_br_expense_amount = 0
        for item in move_with_expense:
    ...
    def _compute_l10n_br_insurance_amount(self):
        move_with_insurrence = self.line_ids.filtered(lambda x: x.l10n_br_is_insurance).mapped('move_id')
        (self - move_with_insurrence).l10n_br_insurance_amount = 0
        for item in move_with_insurrence:
...
class AccountMoveLine(models.Model):
    ...
    @api.depends(
        "debit", "credit", "account_type", "amount_residual"
    )
    def _compute_payment_value(self):
        for item in self:
            item.l10n_br_payment_value = (
                item.debit
                if item.account_type == "receivable"
                else item.credit * -1
            )
