...
class AccountMove(models.Model):
    ...
    def _compute_receivables(self):
        for move in self:
            move.receivable_move_line_ids = move.line_ids.filtered(
                lambda m: m.account_id.account_type == 'asset_receivable'
    ...
    def _compute_payables(self):
        for move in self:
            move.payable_move_line_ids = move.line_ids.filtered(
                lambda m: m.account_id.account_type == 'liability_payable')
    ...
    def validate_data_iugu(self):
        ...
        for invoice in self:
            ...
            if partner.country_id != self.env.ref('base.br') or not invoice.payment_journal_id.receive_by_iugu:
                continue
