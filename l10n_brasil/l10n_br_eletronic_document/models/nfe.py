...
class EletronicDocument(models.Model):
    ...
    def _compute_format_danfe_key(self):
        for item in self:
            chave_nfe_danfe = None
            if item.chave_nfe:
                chave_nfe_danfe = re.sub("(.{4})", "\\1.",
                                          item.chave_nfe, 10, re.DOTALL)
            item.chave_nfe_danfe = chave_nfe_danfe
    ...
    def _hook_validation(self):
        ...
        if not self.company_id.partner_id.country_id:
            ...
            if not self.company_id.partner_id.country_id.ibge_code:
        ...
        if responsavel_tecnico:
            if not responsavel_tecnico.vat:
        ...
        if self.model == 'nfe':
            ...
            if self.company_id.l10n_br_accountant_id and not \
                self.company_id.l10n_br_accountant_id.vat:
        ...
        if not partner.country_id:
            ...
            if not partner.country_id.ibge_code:
 ...
 SEM VALOR FISCAL'
        ...
        prod = {
            ...
            'uTrib': '{:.6}'.format(item.product_id.l10n_br_ncm_id.unidade_tributacao or ''),
        ...
        if item.tipo_produto == 'service':
            ...
            imposto.update({
                'ISSQN': {
                    ...
                    'cMunFG': "%s" % invoice.company_id.city_id.ibge_code,
                    ...
                    #                   invoice.company_id.city_id.ibge_code),
                    'cMun': "%s" % invoice.company_id.city_id.ibge_code,
                    'indIncentivo': 2,  # Não
                }
            })
        else:
    ...
    def _prepare_eletronic_invoice_values(self):
        ...
        ide = {
            ...
            'cMunFG': "%s" % self.company_id.city_id.ibge_code,
        ...
        emit = {
            ...
            'enderEmit': {
                ...
                'nro': self.company_id.partner_id.number,
                'xCpl': self.company_id.street2 or '',
                'xBairro': self.company_id.district,
                'cMun': '%s' % self.company_id.partner_id.city_id.ibge_code,
            ...
            'IE': re.sub('[^0-9]', '', self.company_id.inscr_est or ''),
            'IEST': re.sub('[^0-9]', '', self.iest or ''),
            'CRT': self.cod_regime_tributario,
        }
        if self.company_id.l10n_br_cnae_main_id and self.company_id.inscr_mun:
        ...
        if self.commercial_partner_id:
            ...
            dest = {
                ...
                'cnpj_cpf': re.sub('[^0-9]', '', partner.vat or ''),
                ...
                'enderDest': {
                    ...
                    'cMun': '%s' % partner.city_id.ibge_code,
                    'xMun': partner.city_id.name,
                    'UF': partner.state_id.code,
                    'CEP': re.sub('[^0-9]', '', partner.zip or ''),
                    'cPais': (partner.country_id.ibge_code or '')[-4:],
 ...
 SEM VALOR FISCAL'
        ...
        if self.company_id.l10n_br_accountant_id:
            autorizados.append({
                'CNPJ': re.sub(
                    '[^0-9]', '', self.company_id.l10n_br_accountant_id.vat)
            })
        ...
        if self.transportadora_id.street:
            end_transp = ("%s %s" % (self.transportadora_id.street,
                                          self.transportadora_id.street2 or ''
                                        )).strip()
        else:
            end_transp = ''
        transp = {
        ...
        cnpj_cpf = re.sub('[^0-9]', '', self.transportadora_id.vat or '')
        ...
        if responsavel_tecnico:
            cnpj = re.sub('[^0-9]', '', responsavel_tecnico.vat)
    ...
    def action_post_validate(self):
        chave_dict = {
            'cnpj': re.sub('[^0-9]', '', self.company_id.vat),
            'estado': self.company_id.state_id.ibge_code,
    ...
    def action_send_eletronic_invoice(self):
        ...
        errors = []
        if self.partner_id.country_id == self.env.ref('base.br'):
        ...
        xml_to_send = base64.b64decode(self.xml_to_send).decode('utf-8')
        ...
        if self.codigo_retorno == '100':
            ...
            self.sudo().write({
                'nfe_processada': base64.b64encode(nfe_proc),
        ...
        if self.state in ['cancel', 'done', 'denied']:
            ...
            if nfe_envio.datas and recibo.datas:
                nfe_proc = gerar_nfeproc(
                    base64.b64decode(nfe_envio.datas).decode('utf-8'),
                    base64.b64decode(recibo.datas).decode('utf-8'),
                )
                self.sudo().write({
                    'nfe_processada': base64.b64encode(nfe_proc),
                    'nfe_processada_name': "NFe%08d.xml" % self.numero,
    ...
    def action_cancel_document(self, context=None, justificativa=None):
        ...
        nfe_processada = base64.b64decode(self.nfe_processada)

        nfe_proc_cancel = gerar_nfeproc_cancel(
            nfe_processada, resp['received_xml'].encode())
        if nfe_proc_cancel:
            self.nfe_processada = base64.b64encode(nfe_proc_cancel)
    ...
    def action_get_status(self):
        ...
        if retorno_consulta.cStat == 101:
            ...
            if nfe_proc_cancel:
                self.sudo().write({
                    'nfe_processada': base64.b64encode(nfe_proc_cancel),
                })
        else:
