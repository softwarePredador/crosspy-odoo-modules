...
class Partner(models.Model):
    ...
    def _check_cnpj_inscr_est(self):
        for record in self.filtered(lambda x: x.country_id.code == 'BR' and x.env.company.account_fiscal_country_id.code == "BR"):
            domain = []
        ...
        for record in self:
            ...
            if not disable_cnpj_ie_validation:
                if record.country_id:
                    ...
                    if country_code:
                        if record.vat and country_code.upper() == "BR":
                            if record.is_company:
                                if not cnpj_cpf.validar(record.vat):
                                    result = False
                                    document = "CNPJ"
                            elif not cnpj_cpf.validar(record.vat):
                                result = False
                                document = "CPF"
                if not result:
